...
**M11 3단계(WYSIWYG) 결정 (2026-05-30, phase 15 step 0·2 — `docs/STRETCH_DECISION_REPORT.md`)**: PoC(`WysiwygPreview.vue` — 실 미리보기 위 라벨·컬럼 width 인플레이스 편집 + 삭제)를 완료했다. 좌측 패널과 body draft 배열 공유로 일관성은 높았으나, **신규 필드 추가·복잡 레이아웃은 좌측 패널 의존**으로 단독 완결성이 부족하고 본격 WYSIWYG(Builder.io/GrapeJS 급)는 별도 솔루션 수준 비용이 든다. **결론: 정식 채택 보류 — M9 폼 UI·M10 드래그로 비개발자 진입 장벽이 이미 해소됨**(PRD §8 지표 검증 완료). PoC 자산은 평가용으로 보존. **재도입 트리거**: ① 미리보기에서 신규 필드 추가까지 명시 요구, ② 폼 UI 로 표현 못 하는 복잡 레이아웃 요구 누적, ③ 외부 빌더 임베드 ROI 가 메타 모델 매핑 비용을 정당화할 때 → `16-wysiwyg` PoC 실 착수.
...
**M11 Stretch 최종 결정 (2026-05-30, phase 15 step 2 — `docs/STRETCH_DECISION_REPORT.md`)**: 위 1차 평가를 종합하여 **도입 보류 + 자체 모델 점진 확장**으로 최종 확정한다. Camunda 도입 비용(~80MB·`ACT_*` 40여 테이블·`/engine-rest` 래퍼 불일치·Cockpit/Tasklist 별도 인증·Job Executor 운영)이 실 수요 0건 대비 과대하다. 자체 확장 우선(별도 phase 후보): SLA `@Scheduled` escalation·`steps[].next`/`condition` 조건 분기. 위 도입 트리거(병렬·서브프로세스 실 수요 3건 누적 또는 모델러 요구 명시) 충족 시 `16-bpmn-camunda` 착수. 자체 단계 엔진(ADR-015)은 deprecate 하지 않고 유지·확장한다.
